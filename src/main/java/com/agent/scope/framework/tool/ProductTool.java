...
public class ProductTool extends AbstractTool {
    ...
    @Tool(name = "search_product",
            description = """
                    搜索 HDL 商城产品列表。按产品名关键词搜索，返回匹配的产品列表（含 productId、产品名、价格、SKU 等）。
    ...
    @Tool(name = "query_product_detail",
            description = """
                    查询 HDL 商城单个产品详情。传入产品 ID 或产品名均可。若传入产品名（如'方悦'），系统自动搜索并转换为真实 productId 后查询详情。返回产品名称、SKU 品号列表（含规格）、配件列表、产品参数等。
